...
import { appearancePalettes, getStandardLayout, IMMERSIVE_WORLDS_ENABLED, normalizeStandardLayout, readableTextColor, type AppearanceMode } from "@/lib/appearance";
...
export function Dashboard() {
  ...
  const [accentColor,setAccentColor]=useState("#f4774e"),[navColor,setNavColor]=useState("#111b34"),[screenHue,setScreenHue]=useState("#f7f5ef"),[hueStrength,setHueStrength]=useState(8);
  const [customTextColors,setCustomTextColors]=useState(false),[textColor,setTextColor]=useState("#18243f"),[mutedTextColor,setMutedTextColor]=useState("#68738a");
  ...
  useEffect(() => {
    const rawStats=readLocalJson<Partial<StudentStats>>("orbit-stats",{}),rawProfile=readLocalJson<LocalStudentProfile|null>("orbit-profile",null),migrated=migrateLocalState(rawProfile,rawStats),saved=migrated.stats as Partial<StudentStats>,savedProfile=migrated.profile as LocalStudentProfile|null;
    const legacyTheme=window.localStorage.getItem("orbit-theme")||savedProfile?.theme||"classic";
    const savedTheme=IMMERSIVE_WORLDS_ENABLED?legacyTheme:normalizeStandardLayout(window.localStorage.getItem("orbit-layout")||legacyTheme);
    if(!IMMERSIVE_WORLDS_ENABLED&&legacyTheme!==savedTheme)window.localStorage.setItem("orbit-last-immersive-theme",legacyTheme);
    const layout=getStandardLayout(savedTheme),savedAppearance=window.localStorage.getItem("orbit-appearance"),savedHueStrength=window.localStorage.getItem("orbit-screen-hue-strength"),parsedHueStrength=Number(savedHueStrength);
    const nextAppearance:AppearanceMode=savedAppearance==="light"||savedAppearance==="dark"||savedAppearance==="system"?savedAppearance:layout.recommendedMode;
    setStats(previous=>({...previous,...saved}));setTheme(savedTheme);window.localStorage.setItem("orbit-layout",savedTheme);
    setSceneMode(window.localStorage.getItem("orbit-scene-mode")==="fixed"?"fixed":"daily");setFixedScene(Number(window.localStorage.getItem("orbit-fixed-scene"))||0);
    setAppearance(nextAppearance);setAccentColor(window.localStorage.getItem("orbit-accent")||layout.accent);setNavColor(window.localStorage.getItem("orbit-nav-color")||layout.nav);setScreenHue(window.localStorage.getItem("orbit-screen-hue")||layout.hue);setHueStrength(savedHueStrength===null||!Number.isFinite(parsedHueStrength)?8:Math.min(24,Math.max(0,parsedHueStrength)));setCustomTextColors(window.localStorage.getItem("orbit-custom-text-colors")==="true");setTextColor(window.localStorage.getItem("orbit-text-color")||"#18243f");setMutedTextColor(window.localStorage.getItem("orbit-muted-text-color")||"#68738a");
    setCustomThemeName(window.localStorage.getItem("orbit-custom-theme-name")||"");setNavCollapsed(window.localStorage.getItem("orbit-nav-collapsed")==="true");setDailyGoal(Number(window.localStorage.getItem("orbit-goal"))||5);setFocusMode(window.localStorage.getItem("orbit-focus")==="true");setTextSize(localStorage.getItem("orbit-text-size")||"Default");setHighContrast(localStorage.getItem("orbit-contrast")==="true");setReduceMotion(localStorage.getItem("orbit-reduce-motion")==="true");setDiagnostic(readLocalJson<DiagnosticResult|null>("orbit-diagnostic",null));setProfile(savedProfile);setEntryComplete(localStorage.getItem("orbit-entry-complete")==="true");localStorage.setItem("orbit-stats",JSON.stringify(saved));if(savedProfile)localStorage.setItem("orbit-profile",JSON.stringify(savedProfile));setHydrated(true);
  }, []);
  ...
  const changeTheme = (next: string) => {
    if(IMMERSIVE_WORLDS_ENABLED){setTheme(next);window.localStorage.setItem("orbit-theme",next);return}
    const layout=getStandardLayout(next);setTheme(layout.id);setAppearance(layout.recommendedMode);setAccentColor(layout.accent);setNavColor(layout.nav);setScreenHue(layout.hue);
    window.localStorage.setItem("orbit-layout",layout.id);window.localStorage.setItem("orbit-theme",layout.id);window.localStorage.setItem("orbit-appearance",layout.recommendedMode);window.localStorage.setItem("orbit-accent",layout.accent);window.localStorage.setItem("orbit-nav-color",layout.nav);window.localStorage.setItem("orbit-screen-hue",layout.hue);
  };
  const changeAppearance=(value:AppearanceMode)=>{setAppearance(value);localStorage.setItem("orbit-appearance-source","manual");localStorage.setItem("orbit-appearance",value)};
  const changeAccentColor=(value:string)=>{setAccentColor(value);localStorage.setItem("orbit-accent",value)};
  const changeNavColor=(value:string)=>{setNavColor(value);localStorage.setItem("orbit-nav-color",value)};
  const changeScreenHue=(value:string)=>{setScreenHue(value);localStorage.setItem("orbit-screen-hue",value)};
  const changeHueStrength=(value:number)=>{const safe=Math.min(24,Math.max(0,value));setHueStrength(safe);localStorage.setItem("orbit-screen-hue-strength",String(safe))};
  const changeCustomTextColors=(value:boolean)=>{setCustomTextColors(value);localStorage.setItem("orbit-custom-text-colors",String(value))};
  const changeTextColor=(value:string)=>{setTextColor(value);localStorage.setItem("orbit-text-color",value)};
  const changeMutedTextColor=(value:string)=>{setMutedTextColor(value);localStorage.setItem("orbit-muted-text-color",value)};
  const applyPalette=(paletteId:string)=>{const palette=appearancePalettes.find(item=>item.id===paletteId);if(!palette)return;changeAccentColor(palette.accent);changeNavColor(palette.nav);changeScreenHue(palette.hue)};
  const resetAppearance=()=>{setHueStrength(8);setCustomTextColors(false);setTextColor("#18243f");setMutedTextColor("#68738a");localStorage.setItem("orbit-screen-hue-strength","8");localStorage.setItem("orbit-custom-text-colors","false");localStorage.setItem("orbit-text-color","#18243f");localStorage.setItem("orbit-muted-text-color","#68738a");changeTheme("classic")};
  ...
  if(!profile)return <OnboardingView seed={entrySeed} onComplete={(student,goal)=>{setProfile(student);changeTheme(student.theme);setDailyGoal(goal);localStorage.setItem("orbit-profile",JSON.stringify(student));localStorage.setItem("orbit-goal",String(goal))}}/>;
  ...
  const environmentBackground=!IMMERSIVE_WORLDS_ENABLED?`color-mix(in srgb, ${screenHue} ${hueStrength}%, ${themePack.plainBackground||"#f7f5ef"})`:themePack.plainBackground||`linear-gradient(90deg, color-mix(in srgb, ${themePack.nav} ${isBreakout?28:48}%, transparent), transparent 82%), url('${themeScene}')`;
  ...
  return (
    <main style={{"--orange":accentColor,"--accent-text":readableTextColor(accentColor),"--nav":navColor,"--nav-text":readableTextColor(navColor),"--screen-hue":screenHue,"--screen-hue-strength":`${hueStrength}%`,"--screen-hue-soft":`${Math.round(hueStrength*.55)}%`,"--custom-ink":textColor,"--custom-muted":mutedTextColor} as CSSProperties} className={`app-shell theme-${theme} motion-${themePack.motion||"drift"} mode-${appearance} room-variant-${roomVariant} ${!IMMERSIVE_WORLDS_ENABLED?"standard-appearance":"immersive-appearance"} ${customTextColors?"custom-text-colors":"automatic-text-colors"} ${navCollapsed?"nav-collapsed":""} ${isBreakout?"breakout-space":"main-space"} ${focusMode?"focus-mode":""} text-${textSize.toLowerCase().replace(" ","-")} ${highContrast?"high-contrast":""} ${reduceMotion?"reduce-motion":""}`}>
      <a className="skip-link" href="#main-content">Skip to main content</a>
      <div className="environment-layer" style={{background:environmentBackground}} aria-hidden="true"><i/><b/><span>{IMMERSIVE_WORLDS_ENABLED&&customThemeName&&theme==="custom"?customThemeName:""}</span></div>
      ...
      <section className="content" id="main-content" tabIndex={-1}>
        ...
        {active === "Settings" && <div className="settings-page-shell"><SettingsView theme={theme} onTheme={changeTheme} sceneMode={sceneMode} sceneIndex={fixedScene} onSceneMode={(value)=>{setSceneMode(value);localStorage.setItem("orbit-scene-mode",value)}} onScene={(value)=>{setFixedScene(value);setSceneMode("fixed");localStorage.setItem("orbit-fixed-scene",String(value));localStorage.setItem("orbit-scene-mode","fixed")}} appearance={appearance} onAppearance={changeAppearance} accentColor={accentColor} navColor={navColor} screenHue={screenHue} hueStrength={hueStrength} customTextColors={customTextColors} textColor={textColor} mutedTextColor={mutedTextColor} onAccentColor={changeAccentColor} onNavColor={changeNavColor} onScreenHue={changeScreenHue} onHueStrength={changeHueStrength} onCustomTextColors={changeCustomTextColors} onTextColor={changeTextColor} onMutedTextColor={changeMutedTextColor} onPalette={applyPalette} onResetAppearance={resetAppearance} customThemeName={customThemeName} onCreateTheme={createTheme} dailyGoal={dailyGoal} onGoal={(goal)=>{setDailyGoal(goal);window.localStorage.setItem("orbit-goal",String(goal))}} focusMode={focusMode} onFocus={(value)=>{setFocusMode(value);window.localStorage.setItem("orbit-focus",String(value))}} /><AccessibilityControls textSize={textSize} highContrast={highContrast} reduceMotion={reduceMotion} onTextSize={value=>{setTextSize(value);localStorage.setItem("orbit-text-size",value)}} onContrast={value=>{setHighContrast(value);localStorage.setItem("orbit-contrast",String(value))}} onMotion={value=>{setReduceMotion(value);localStorage.setItem("orbit-reduce-motion",String(value))}}/><AccountControls profile={profile} onSignOut={()=>{localStorage.removeItem("orbit-entry-complete");setEntryComplete(false);navigate("Dashboard",true)}}/><QualityEntry/><DataControls profile={profile} stats={stats} diagnostic={diagnostic} onReset={()=>{setStats(emptyStats);localStorage.setItem("orbit-stats",JSON.stringify(emptyStats))}} /></div>}
