...
export function Dashboard() {
  ...
  const resetAppearance=()=>{setHueStrength(8);localStorage.setItem("orbit-screen-hue-strength","8");changeTheme("classic")};
  ...
  return (
    <main style={{"--orange":accentColor,"--accent-text":readableTextColor(accentColor),"--nav":navColor,"--nav-text":readableTextColor(navColor),"--screen-hue":screenHue,"--screen-hue-strength":`${hueStrength}%`,"--screen-hue-soft":`${Math.round(hueStrength*.55)}%`} as CSSProperties} className={`app-shell theme-${theme} motion-${themePack.motion||"drift"} mode-${appearance} room-variant-${roomVariant} ${!IMMERSIVE_WORLDS_ENABLED?"standard-appearance":"immersive-appearance"} ${navCollapsed?"nav-collapsed":""} ${isBreakout?"breakout-space":"main-space"} ${focusMode?"focus-mode":""} text-${textSize.toLowerCase().replace(" ","-")} ${highContrast?"high-contrast":""} ${reduceMotion?"reduce-motion":""}`}>
